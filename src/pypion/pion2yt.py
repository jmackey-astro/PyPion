...
##########################################################################
  def get_ds(self, filename, quantities=["density"], **kwargs) -> yt.data_objects.static_output.Dataset:
...
##########################################################################
  def get_ds3D(self, data, quantities=["density"], **kwargs) -> yt.data_objects.static_output.Dataset:
...
##########################################################################
  def get_ds2D(self, data, quantities=["density"], **kwargs) -> yt.data_objects.static_output.Dataset:
      sim_time = data.sim_time()
      N_levels = data.nlevels()
      Dom_size = max(np.array(data.level_max()) - np.array(data.level_min()))
...
# snapshots.
  def get_ts(self, selfevolution, **kwargs):
